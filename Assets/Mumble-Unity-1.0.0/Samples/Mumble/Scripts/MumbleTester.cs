...
namespace Mumble.Sample
{
    public class MumbleTester : MonoBehaviour
    {
        ...
        {
            yield return new WaitUntil(() => permitirConexion);
            while (!_mumbleClient.ReadyToConnect)
                yield return null;
            if (debugEnConsola) Debug.Log("Will now connect");
            _mumbleClient.Connect(Username, Password);
            yield return null;
            if (MyMumbleMic != null)
            {
        ...
        private MumbleAudioPlayer CreateMumbleAudioPlayerFromPrefab(string username, uint session)
        {
            ...
            GameObject newObj = SendPosition
                ? Instantiate(MyMumbleAudioPlayerPositionedPrefab)
                : Instantiate(MyMumbleAudioPlayerPrefab);

            newObj.name = username + "_MumbleAudioPlayer";
            MumbleAudioPlayer newPlayer = newObj.GetComponent<MumbleAudioPlayer>();
            if(debugEnConsola) Debug.Log("Adding audio player for: " + username);
            return newPlayer;
        }
        ...
        {
            if (debugEnConsola) print("User #" + session + " had their user state change");
        ...
        IEnumerator ExampleMicReconnect(string micToConnect)
        {
            while (true)
            {
                ...
                for (int i = 0; i < micNames.Length; i++)
                {
                    ...
                    {
                        if (debugEnConsola) Debug.Log("Desired mic reconnected");
        ...
        {
            if (debugEnConsola) Debug.LogWarning("Shutting down connections");
            _mumbleClient?.Close();
        }
        ...
        void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.S))
            {
                ...
                if (debugEnConsola) print("Sent mumble message");
            }
            if (Input.GetKeyDown(KeyCode.J))
            {
                if (debugEnConsola) print("Will attempt to join channel " + ChannelToJoin);
                _mumbleClient.JoinChannel(ChannelToJoin);
            }
            if (Input.GetKeyDown(KeyCode.Escape))
            {
                if (debugEnConsola) print("Will join root");
                _mumbleClient.JoinChannel("Root");
            }
            if (Input.GetKeyDown(KeyCode.C))
            {
                if (debugEnConsola) print("Will set our comment");
                _mumbleClient.SetOurComment("Example Comment");
            }
            if (Input.GetKeyDown(KeyCode.B))
            {
                if (debugEnConsola) print("Will set our texture");
            ...
            if (Input.GetKeyDown(KeyCode.UpArrow))
            {
                ...
                if (debugEnConsola) Debug.Log("Increasing bitrate " + currentBW + "->" + newBitrate);
            ...
            if (Input.GetKeyDown(KeyCode.DownArrow))
            {
                int currentBW = MyMumbleMic.GetBitrate();
                int newBitrate = currentBW - BandwidthChange;
                if (debugEnConsola) Debug.Log("Decreasing bitrate " + currentBW + "->" + newBitrate);
                MyMumbleMic.SetBitrate(newBitrate);
            }
        }
    }
